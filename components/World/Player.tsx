...
export const Player: React.FC = () => {
  ...
  // Virtual Controls (Custom Events)
  useEffect(() => {
    if (status !== GameStatus.PLAYING) return;

    const handleLeft = () => setLane(l => Math.max(l - 1, -Math.floor(laneCount / 2)));
    const handleRight = () => setLane(l => Math.min(l + 1, Math.floor(laneCount / 2)));
    const handleJump = () => triggerJump();
    const handleAction = () => activateImmortality();

    window.addEventListener('control-left', handleLeft);
    window.addEventListener('control-right', handleRight);
    window.addEventListener('control-jump', handleJump);
    window.addEventListener('control-action', handleAction);

    return () => {
        window.removeEventListener('control-left', handleLeft);
        window.removeEventListener('control-right', handleRight);
        window.removeEventListener('control-jump', handleJump);
        window.removeEventListener('control-action', handleAction);
    };
  ...
  useEffect(() => {
    ...
    const handleTouchEnd = (e: TouchEvent) => {
        ...
        if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > 30) {
            ...
            // Only trigger action if not hitting a button (simple heuristic, though buttons stop propagation usually)
            // But we keep this for screen taps elsewhere
            activateImmortality();
        }
    };
