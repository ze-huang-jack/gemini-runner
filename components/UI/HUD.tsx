...
const MobileControls: React.FC = () => {
    const { activateImmortality, hasImmortality, isImmortalityActive } = useStore();
    ...
    return (
        <div className="absolute inset-0 pointer-events-none z-50 flex flex-col justify-end pb-8 px-6">
            <div className="flex justify-between items-end w-full max-w-5xl mx-auto">
                {/* Movement Controls */}
                <div className="flex gap-4 pointer-events-auto">
                    <button 
                        className="w-16 h-16 bg-white/5 backdrop-blur-sm border border-white/20 rounded-full flex items-center justify-center active:bg-cyan-500/30 active:scale-90 transition-all shadow-[0_0_15px_rgba(0,0,0,0.5)] touch-none"
                        onPointerDown={(e) => { e.preventDefault(); dispatch('control-left'); }}
                    >
                        <ArrowLeft className="w-8 h-8 text-cyan-100" />
                    </button>
                    <button 
                         className="w-16 h-16 bg-white/5 backdrop-blur-sm border border-white/20 rounded-full flex items-center justify-center active:bg-cyan-500/30 active:scale-90 transition-all shadow-[0_0_15px_rgba(0,0,0,0.5)] touch-none"
                         onPointerDown={(e) => { e.preventDefault(); dispatch('control-right'); }}
                    >
                ...
                <div className="flex gap-4 items-end pointer-events-auto">
                    {hasImmortality && (
                        <button 
                             className={`w-12 h-12 mb-2 bg-yellow-500/10 backdrop-blur-sm border ${isImmortalityActive ? 'border-yellow-400 bg-yellow-500/40' : 'border-yellow-500/30'} rounded-full flex items-center justify-center active:scale-90 transition-all shadow-[0_0_10px_rgba(255,215,0,0.2)] touch-none`}
                             onPointerDown={(e) => { e.preventDefault(); activateImmortality(); }}
                        >
                            <Shield className={`w-6 h-6 ${isImmortalityActive ? 'text-white' : 'text-yellow-400'}`} />
                        </button>
                    )}
                    <button 
                         className="w-20 h-20 bg-gradient-to-tr from-cyan-900/50 to-cyan-500/20 backdrop-blur-md border border-cyan-400/30 rounded-full flex items-center justify-center active:bg-cyan-400/30 active:scale-90 transition-all shadow-[0_0_20px_rgba(0,255,255,0.2)] touch-none"
                         onPointerDown={(e) => { e.preventDefault(); dispatch('control-jump'); }}
                    >
  ...
  return (
    <>
      <div className={containerClass}>
          ...
          {/* Bottom Overlay */}
          <div className="w-full flex justify-end items-end mb-20 md:mb-0">
